...
class ViewActivityController extends Controller
{
    ...
    public function index()
    {
        ...
        //                 ->leftJoin('likes as lk', 'pc.id', '=', 'lk.post_content_id')
        //                 ->leftJoin('comments as cm', 'pc.id', '=', 'cm.post_content_id')
        //                 ->leftJoin('sub_comments as sbcm', 'pc.id', '=', 'sbcm.post_content_id')
        //                 ->select('pc.*',
        //                         DB::raw('COUNT(lk.post_content_id) as like_count'),
        //                         DB::raw('COUNT(cm.post_content_id) as comment_count')
     ...
     */
    public function show(PostContent $PostContent,$postId)
    {
        ...
        $comments = DB::table('post_contents as pc')
            ...
            ->select('cm.*','users.first_name','users.last_name','sbcm.sub_comment_text')
            ->where('pc.id', $postId)
        ->get();
